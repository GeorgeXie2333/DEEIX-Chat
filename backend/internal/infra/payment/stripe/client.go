...
// CheckoutInput 与 CheckoutResult 保留旧包级名称，实际契约由 ports/payment 统一维护。
type CheckoutInput = paymentport.StripeCheckoutInput
type CheckoutResult = paymentport.CheckoutResult
...
func checkoutForm(input paymentport.StripeCheckoutInput) url.Values {
	...
	form.Set("metadata[pay_subtotal_amount_cents]", strconv.FormatInt(input.PaySubtotalAmountCents, 10))
	form.Set("metadata[pay_amount_cents]", strconv.FormatInt(input.PayAmountCents, 10))
	form.Set("metadata[fee_rate_basis_points]", strconv.FormatInt(input.FeeRateBasisPoints, 10))
	form.Set("metadata[fee_rate_percent]", strconv.FormatFloat(float64(input.FeeRateBasisPoints)/100, 'f', -1, 64))
	form.Set("metadata[fee_amount_cents]", strconv.FormatInt(input.FeeAmountCents, 10))
