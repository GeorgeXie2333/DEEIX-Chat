...
func TestCreateCheckoutSessionMapsProtocolAndValidatesResponse(t *testing.T) {
	client := &Client{httpClient: &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		if form.Get("metadata[user_id]") != "42" || form.Get("metadata[pay_amount_cents]") != "2059" {
			t.Fatalf("missing checkout metadata: %#v", form)
		}
		if form.Get("line_items[0][price_data][unit_amount]") != "2059" || form.Get("line_items[1][price_data][unit_amount]") != "" {
			t.Fatalf("expected a single fee-inclusive line item: %#v", form)
		}
		if form.Get("metadata[pay_subtotal_amount_cents]") != "1999" ||
			form.Get("metadata[fee_rate_basis_points]") != "300" ||
			form.Get("metadata[fee_rate_percent]") != "3" ||
			form.Get("metadata[fee_amount_cents]") != "60" {
			t.Fatalf("missing fee metadata: %#v", form)
		}
