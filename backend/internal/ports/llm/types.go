...
type GenerateOutput struct {
	...
	// StreamState 供适配器在单次流式解析期间保存私有聚合状态，不进入传输或持久化契约。
...
// RawChatCompletionOutput 保留 OpenAI Chat Completions 原始响应，同时附带统一解析结果。
...
// ChatClient 是应用层可使用的统一生成与原始 Chat Completions 端口。
type ChatClient interface {
	...
	GenerateRawChatCompletion(ctx context.Context, route RouteConfig, body map[string]interface{}) (*RawChatCompletionOutput, error)
	GenerateRawChatCompletionStream(ctx context.Context, route RouteConfig, body map[string]interface{}, onEvent func(RawChatCompletionStreamEvent) error) (*RawChatCompletionOutput, error)
}
