...
func TestFilterModelOptionsPreservesGeminiNativeTools(t *testing.T) {
	...
	if _, ok := tools[2]["url_context"]; !ok {
		t.Fatalf("expected url_context tool third, got %#v", tools[2])
	}
}
...
func TestFilterModelOptionsGeminiPolicyKeyMatchesGoogleAdapter(t *testing.T) {
	...
	if _, ok := tools[0]["google_search"]; !ok {
		t.Fatalf("expected Gemini google_search payload, got %#v", tools[0])
	}
}
...
func TestFilterModelOptionsGeminiInteractionsAllowsVideoParams(t *testing.T) {
	...
	if !ok || responseFormat["aspect_ratio"] != "16:9" || responseFormat["duration"] != "7s" || responseFormat["image_size"] != "1K" || responseFormat["mime_type"] != "image/jpeg" {
